#!/usr/bin/env python3
"""
Run shared-memory stride experiments and capture Nsight Compute reports.

This script mirrors run_stride_cache_experiment.py but focuses on shared memory
bank behavior. It compiles shared_stride_benchmark.cu and launches Nsight
Compute with --set full for stride values between 4 and 128 bytes (step 4).
Each run stores the .ncu-rep and .log artifacts under ncu_shared_stride_reports/.
"""
...
CUDA_FILE = SCRIPT_DIR / "shared_stride_benchmark.cu"
BINARY_PATH = SCRIPT_DIR / "shared_stride_benchmark"
REPORT_DIR = SCRIPT_DIR / "ncu_shared_stride_reports"
...
def run_ncu_for_stride(
    ...
    report_base = REPORT_DIR / f"shared_stride_{stride_bytes:03d}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark shared-memory stride accesses with Nsight Compute.",
    )
    parser.add_argument(
        "--nvcc",
        default="nvcc",
    ...
    parser.add_argument(
        ...
        help="Stride sizes (bytes) to test. Defaults to 4..128 step 4.",
    )
    parser.add_argument(
        "--ncu-set",
        type=str,
...
def main() -> int:
    ...
    print("\nShared-memory stride experiments complete. Reports saved:")
